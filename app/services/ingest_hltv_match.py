...
def get_or_create_team(db: Session, name: str) -> Team:
    ...
    team = Team(name=name)
    db.add(team)
    db.flush()  # assigns team.id without commit
    return team


def ingest_hltv_match(db: Session, payload: dict) -> None:
    match_id = int(payload["match_id"])
    team1_name = payload["team1_name"]
    team2_name = payload["team2_name"]

    t1 = get_or_create_team(db, team1_name)
    t2 = get_or_create_team(db, team2_name)

    played_at = int(payload["timestamp"])  # unix seconds
    ...
    # Replace veto actions for this match
    ...
    # Replace map results for this match
